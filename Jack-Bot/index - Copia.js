...
// === Carrega todos os plugins (versão robusta) ===
function loadComandos() {
  comandos.clear()
  const comandosRoot = path.join(__dirname, 'comandos')
  ...
  function percorrer(dir) {
    fs.readdirSync(dir).forEach(file => {
      const fullPath = path.join(dir, file)
      if (fs.lstatSync(fullPath).isDirectory()) {
        // subpasta: percorre recursivamente
        percorrer(fullPath)
      } else if (file.endsWith('.js')) {
        try {
          delete require.cache[require.resolve(fullPath)]
        ...
        let comandoModule
        try {
          comandoModule = require(fullPath)
        } catch (err) {
          console.error(`[✘] Erro ao carregar ${fullPath}:`, err)
          return
        }

        // Determina permissões com base na pasta (dono/adm/vip/geral)
        const relative = path.relative(comandosRoot, fullPath)
        const parts = relative.split(path.sep)
        let permissao = 'geral'
        if (parts.length > 1) {
          const p = parts[0].toLowerCase()
          if (p === 'dono') permissao = 'dono'
          else if (p === 'adm') permissao = 'adm'
          else if (p === 'vip') permissao = 'vip'
        }
        comandoModule.permissao = permissao

        // Normaliza aliases: aceita { comandos: ['a','b'] } ou { comando: 'a' } ou { nome: 'a' }
        let aliases = []
        if (Array.isArray(comandoModule.comandos) && comandoModule.comandos.length) {
          aliases = comandoModule.comandos
        } else if (typeof comandoModule.comando === 'string' && comandoModule.comando.trim()) {
          aliases = [comandoModule.comando.trim()]
        } else if (typeof comandoModule.nome === 'string' && comandoModule.nome.trim()) {
          aliases = [comandoModule.nome.trim()]
        } else {
          console.warn(`[!] Módulo ${file} não exporta 'comandos' (array), 'comando' (string) nem 'nome' (string). Ignorando.`);
          return
        }

        // Registra aliases no Map
        aliases.forEach(cmd => {
          if (!cmd || typeof cmd !== 'string') return
          comandos.set(cmd.toLowerCase(), comandoModule)
        })
      }
    })
  }
  ...
  try {
    percorrer(comandosRoot)
    console.log(`[✓] Comandos carregados: ${[...comandos.keys()].length}`)
  } catch (err) {
    console.error("[✘] Erro ao percorrer a pasta de comandos:", err)
  }
}
...
try { bio = (await lux.fetchStatus(sender)).status } catch { bio = "Charliin é Menor Quente" }
...
//MENSAGENS RÁPIDAS
const msg = {
 espera: "- *calma irmãozin, ta carregando... *",
 dono: "- *Você não me parece com o Charliin tralha... ��*",
 query: "- *Procurei essa bomba que tu disse, mas não achei nada..*",
 adm: "- *Só os ADM que pode, sai fora ivamembro *",
 admBot: "- *To sem poderes, me da adm pra ativar as funções :(*",
 vip: "- *Apenas V.I.P's podem usar isso, você não é um. *",
}
...
lux.sendMessage(from, { text: `- *Não funcionou...*\nToque *${prefix}menu* e veja oq funciona ae bundao*` }, { quoted: selo });
}
}
} catch (e) {
console.log(e);
